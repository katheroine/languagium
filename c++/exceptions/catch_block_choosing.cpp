...
class TwoThousandException : public NumberValueException {
public:
  TwoThousandException(int number) {
    this->number = number;
    message = "2000 number has beign given.";
  }
};
...
int drawNumber() {
  ...
  if (number == 0) {
    ...
    TwoThousandException *exception = new TwoThousandException(number);
    throw exception;
...
int main() {
  ...
  } catch (TwoThousandException *e) {
    cout << "CASE 4: " << e->message << " (" << e->number << ")" << endl;
  }
