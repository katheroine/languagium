...
constructor NumberValueException.create(exception_number: integer);
begin
...
begin
  ...
  if (number = 0) then
    ...
    raise NumberValueException.create(number)
...
begin
  ...
  try
    ...
    on e: NumberValueException do writeln('CASE 0: ', e.message, ' (', e.number, ')');
    on e: ThousandException do writeln('CASE 3: ', e.message, ' (', e.number, ')'); // Will be never catched.
  end;
